...
router.post("/login", async (req, res) => {
	...
	try {
		userExistent= await Users.findOne({where:{username}})
			
		 console.log(userExistent)

		if (userExistent) {
			if (await bcrypt.compare(password,userExistent.password)) {
				
				res.status(200).json({ message: 'Succesfuly logged in!' ,userId:userExistent.id})
			}
			else {
				res.json({ message: 'Password incorrect.' })
			}
		}else {
			res.json({ message: 'User with this username doesnt exists.' })
		}
...
router.put('/:UserId', async (req, res) => {
	...
	var fieldsUpdate=['username','firstName','lastName','city'];
	const {username, password, firstName, lastName, city } = req.body;
